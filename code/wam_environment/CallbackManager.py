...
class CallbackManager:
    def __init__(self, tracking_ball=True, tracking_robot=True, tracking_debug_image=False):
        self.ball_position = []
        self.wam_position = []
        self.wam_speed = []
        ...
        if tracking_debug_image:
            rospy.Subscriber('/ball_detection_1/debug_img', Image, self._image_callback_1)
            rospy.Subscriber('/ball_detection_2/debug_img', Image, self._image_callback_2)
    ...
    def stop_recording(self):
        if self.record_on:
            ...
            print("[Warning] Requested stop_recording but recording is not on. ")

    def _ball_callback(self, data: Odometry):
        self.ball_position = [data.pose.pose.position.x, data.pose.pose.position.y, data.pose.pose.position.z]

    def _wam_callback(self, data: JointState):
        self.wam_position = translate_from_joint_state_order_to_moveit_order(list(data.position))
        self.wam_speed = translate_from_joint_state_order_to_moveit_order(list(data.velocity))
        self.time = time.time()
        ...
        if self.tracking_debug_image:
            if len(self.record_image_data_1) > 0:
                ...
                for i in range(len(self.record_image_data_1)):
                    cv2.imwrite(temp_image_path + "%d.jpeg" % i, self.record_image_data_1[i])
                save_video(self.record_image_data_1, temp_image_path + "video.mp4")
                ...
                for i in range(len(self.record_image_data_1)):
                    cv2.imwrite(demo_image_path + "%d.jpeg" % i, self.record_image_data_1[i])
                save_video(self.record_image_data_1, demo_image_path + "video.mp4")
            ...
            if len(self.record_image_data_2) > 0:
                ...
                for i in range(len(self.record_image_data_2)):
                    cv2.imwrite(temp_image_path + "%d.jpeg" % i, self.record_image_data_2[i])
                save_video(self.record_image_data_2, temp_image_path + "video.mp4")
                print("Demo images/videos saved at " + temp_image_path)

                demo_image_path = "demos/images/" + saving_name + "/2/"
                os.makedirs(demo_image_path, exist_ok=True)
                for i in range(len(self.record_image_data_2)):
                    cv2.imwrite(demo_image_path + "%d.jpeg" % i, self.record_image_data_2[i])
                save_video(self.record_image_data_2, demo_image_path + "video.mp4")
                print("Demo images/videos saved at " + demo_image_path)
    ...
    def save_record_data_for_ball_calculation(self, saving_name):
        print("Saving replay data...")
        log_dir = "analysis/record_data_for_ball_calculation/" + saving_name + "/"
